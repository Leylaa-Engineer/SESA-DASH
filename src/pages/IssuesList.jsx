...
function groupIssuesByMachine(issueList) {
  ...
  issueList.forEach((issue) => {
    ...
    const latestTime = issue.olusturulma_tarihi?.toMillis?.() || 0;
    const existingTime = existing?.olusturulma_tarihi?.toMillis?.() || 0;
  ...
  return [...grouped.values()].sort((a, b) => (b.olusturulma_tarihi?.toMillis?.() || 0) - (a.olusturulma_tarihi?.toMillis?.() || 0));
}
  ...
  useEffect(() => {
    const fetchIssues = async () => {
      ...
      try {
        let list;
        if (isAdmin) {
          // Filtreyi makine bazında uygulayabilmek için tüm kayıtlar alınır.
          list = (await getDocs(collection(db, 'arizalar'))).docs.map((item) => ({ id: item.id, ...item.data() }));
        ...
        const machineIssues = groupIssuesByMachine(list);
